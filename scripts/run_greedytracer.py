...
def save_standard_panel(
    img_rgb, vesselness, gt_skel_vis, pred_skel_vis, mask, res, image_id, panels_dir
):
    ...
    axes[1].set_title(
        "Frangi Vesselness Map", fontweight="bold", fontsize=FONT_SIZE_TITLE
    )
...
def save_trajectory_panel(
    ...
    axes[0].set_title(
        f"Vesselness + {N} Seeds", color="white", fontsize=FONT_SIZE_TITLE
    )
    ...
    cbar.set_label(
        "Visit Order (0 = First/Strongest)", color="white", fontsize=FONT_SIZE_LABEL
    )
    ...
    axes[2].set_ylabel(
        "Count (Number of Traces)", color="white", fontsize=FONT_SIZE_LABEL
    )
...
def evaluate(dataset_name):
    ...
    for i in tqdm(range(num_total), desc=f"Greedy Tracer — {dataset_name}"):
        sample = dataset[i]
        image_id = sample["id"]
